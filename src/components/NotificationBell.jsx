...
const NotificationBell = ({ notificationCount, notifications, setNotifications }) => {
    ...
    const markAsRead = async (id) => {
        try {
            const response = await axios.patch(`http://localhost:5000/api/notifications/notifications/${id}`, { read: true },
    ...
    return (
        <div className="relative cursor-pointer">
            <button className="focus:outline-none cursor-pointer" onClick={toggleDropdown}>
                <Bell size={24} />
                {notificationCount > 0 && (
                            ...
                            notifications.map((notification) => (
                                ...
                                >
                                    <div className={`flex-1 w-100 ${notification.isRead ? 'text-gray-500' : 'font-bold'} text-sm`}>
                                        {notification.message} {/* Adjust this based on your notification structure */}
                                        {notification.isRead ? null : (
                                            <span className="absolute -top-1 -left-1 bg-red-500 text-white font-bold rounded-full px-2"
